...
; Pinout:
; pin  1 PORTA2	NR Input	Vref-	battery ground	
; pin  2 PORTA3	NR Input	Vref+	10.24V 
; pin  3 PORTA4
; pin  4 PORTA5	MCLR (is input only)
; pin  5 VSS		GND
; pin  6 PORTB0	Output 	LED voltage drop
; pin  7 PORTB1	Output	LED voltage constant
; pin  8 PORTB2	Output	LED voltage rise	
; pin  9 PORTB3	Output	Buzzer after 5 drop
; pin 10 PORTB4
; pin 11 PORTB5
; pin 12 PORTB6
; pin 13 PORTB7
...
;Main program
	...
	;MOVWF	TRISA
	CLRF	TRISA
	BSF	TRISA, 0    ;porta bit 0 is input

	BANK0

	CLRF	PORTB		;Turn LEDs on but not the buzzer
	BSF	PORTB, 0
	BSF	PORTB, 1
	BSF	PORTB, 2
	...
	BSF	ANSEL, 0	;PORTA0 AN0 analog
	;BSF	ANSEL, 2	;vref- is VSS
	;BSF	ANSEL, 3	;vref+ is VPP
...
wait_pool:
	CALL WAIT_1s	;wait 0.5 sec before checking voltage
...
;		GOTO wait_pool_sub
	...
	CLRF PORTB	
	CALL WAIT_20ms; show all dels off for a short time
	; check voltage
	CALL ADC
...
GT:
	BSF PORTB, 2 ; high	green	
	MOV_short Result, last_Res	;save new value
	MOVLW 5	;reset count
	MOVWF count
	BCF PORTB, 3 ;reset buzzer 
	GOTO wait_pool
...
LT:
	...
	DECFSZ count, F	
	GOTO wait_pool
	BSF PORTB, 3 ; buzzer after 5x LT
	MOVLW 2	; soft reset count
	MOVWF count
	GOTO wait_pool
	
EQ:
	BSF PORTB, 1 ; eq	yellow
	;BCF PORTB, 3 ;reset buzzer 
	GOTO wait_pool
...
WAIT_20ms:
	MOVLW	37		;wait cycles - 2 ; 37 + 2 = 39, *4 = 156, *0.000128 = 0.019968s
	MOVWF	Wait20ms
	NOP
	NOP
