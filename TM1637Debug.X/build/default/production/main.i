...
;#############################################################################
 ...
 BTFBS PIR1, ((PIR1) and 07Fh), 0, ISR_T1

 GOTO ISR_END ; unkown interrupt

ISR_T1:
 BCF PIR1, ((PIR1) and 07Fh), 0
...
SETUP:
 ...
 ; init osc 8MHz
 BCF OSCCON, ((OSCCON) and 07Fh), 0
 BCF OSCCON, ((OSCCON) and 07Fh), 1
 BSF OSCCON, ((OSCCON) and 07Fh), 4
 BSF OSCCON, ((OSCCON) and 07Fh), 5
 BSF OSCCON, ((OSCCON) and 07Fh), 6

WAIT_OSC:
 BTFSS OSCCON, ((OSCCON) and 07Fh), 2
 GOTO WAIT_OSC
 ...
 CLRF TMR1L
 CLRF TMR1H
 BCF T1CON, ((T1CON) and 07Fh), 4 ;
 BCF T1CON, ((T1CON) and 07Fh), 5 ;
 BCF T1CON, ((T1CON) and 07Fh), 1 ; timer clock is FOSC/4
 BSF T1CON, ((T1CON) and 07Fh), 0 ; timer ON

 MOVLW 0xFF
 MOVWF PORTB ; all high

; disable interrupts
 BCF INTCON, ((INTCON) and 07Fh), 6 ; peripheral int
 BCF INTCON, ((INTCON) and 07Fh), 7 ; global int
